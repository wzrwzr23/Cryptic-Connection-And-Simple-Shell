...
def main(args):
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        while state:
            session_key_bytes = Fernet.generate_key() # generates 128-bit symmetric key as bytes(16 bytes)
            ...
            s.sendall(convert_int_to_bytes(len(enc_key)))
            s.sendall(enc_key)

            # rec_key_len = convert_bytes_to_int(read_bytes(s, 8))
            # rec_key = read_bytes(s, rec_key_len)

            # assert rec_key == enc_key
